...
function calculateScore(item, query, preferences) {
  ...
  if (preferences.chicken && /\bchicken\b/i.test(text)) {
    score += 50;
  }
  ...
  if (preferences.pizza && /\bpizza\b/i.test(text)) {
    score += 50;
  }

  if (preferences.burger && /\bburger\b/i.test(text)) {
    score += 50;
  }

  if (preferences.pasta && /\bpasta\b/i.test(text)) {
    score += 50;
  }

  if (preferences.biryani && /\bbiryani\b/i.test(text)) {
    score += 50;
  }

  if (preferences.salad && /\bsalad\b/i.test(text)) {
    score += 50;
  }

  if (
    preferences.highProtein &&
    /\bprotein\b|\begg\b|\bpaneer\b|\bchicken\b/i.test(text)
  ) {
    score += 30;
  }
  ...
  if (
    preferences.spicy &&
    /\bspicy\b|\bhot\b|\bchilli\b|\bmasala\b/i.test(text)
  ) {
    score += 25;
  }
...
function getCandidates(availableFoods, query) {
  ...
  let candidates = hasStrongPreference
    ? availableFoods.filter((item) =>
        matchesStrongKeyword(item, preferences)
      )
    : [...availableFoods];
  ...
  candidates = candidates
    .map((item) => ({
      item,
      score: calculateScore(item, query, preferences),
    }))
...
async function toolSearchCatalog({
  ...
  return items.map((it) => ({
    ...
    restaurantName:
      it.restaurant?.name || 'Partner Restaurant',
    image: it.image || it.restaurant?.image || null,
  }));
}
...
async function toolInspectAndOptimizeCart({
  ...
  cartItems.forEach((cartItem) => {
    const cartItemId =
      cartItem.menuItem || cartItem._id;
...
async function toolGenerateGroupMealPlan({
  ...
  const catalog = await MenuItem.find(filter)
    .populate('restaurant', 'name deliveryTime image')
    .lean();
  ...
  const vegItems = catalog
    .filter((i) => i.isVeg)
    .sort((a, b) => a.price - b.price);

  const nonVegItems = catalog
    .filter((i) => !i.isVeg)
    .sort((a, b) => a.price - b.price);
    ...
    for (const item of vegItems) {
      if (
        ...
        proposedItems.push({
          ...
          image: item.image || item.restaurant?.image || null,
        });
    ...
    preferences.forEach(
      (member, idx) => {
        ...
        memberRecommendations.push({
          ...
          reason:
            `Matches ${member.foodPreference || 'all'} dietary choice & ₹${chosen.price} personal budget.`,
        });
    ...
    if (budgetRemaining >= 100) {
      ...
      if (addOns.length > 0) {
        sharedSuggestions.push({
          ...
          image: addOns[0].image || groupOrder.restaurant.image || null,
...
${
  groupOrderContext
    ? JSON.stringify(groupOrderContext)
    : 'None'
}
`;
    ...
    if (
      ...
      // -------------------------------------------------------
        ...
        addAudit(
          'Group Plan Engine',
          `Composed meal for ${
            call.args?.totalPeople || 0
          } people totaling ₹${
            toolCallResult.calculatedTotal || 0
          }`
        );
      }
      ...
      // -------------------------------------------------------
      // IMPORTANT GEMINI FUNCTION-CALL FOLLOW-UP
      //
      // We preserve Gemini's ORIGINAL model content instead
      // of manually rebuilding the functionCall part.
      //
      // We also return the exact call.id.
      // -------------------------------------------------------
      ...
      const followUp =
        await ai.models.generateContent({
          ...
          contents: [
            ...
            // IMPORTANT:
            // Preserve Gemini's actual model response.
            modelContent,
    ...
    // ---------------------------------------------------------
    // NO TOOL CALL FALLBACK (FALLBACK TO DIRECT CATALOG SEARCH)
    // ---------------------------------------------------------

    else {
      toolCallResult = await toolSearchCatalog({ query: prompt });

      addAudit(
        'Catalog Fallback',
        `Executed direct catalog fallback, retrieved ${toolCallResult.length} dishes`
      );

      finalAssistantText = toolCallResult.length > 0
